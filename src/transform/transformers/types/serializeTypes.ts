...
export function serializeLocalJSXDeclaration(
  ...
  return [
    ...
    '  interface IntrinsicElements {',
    components
      .map(c => `    "${c.tagName}": ${dashToPascalCase(c.tagName)};`)
      .join('\n'),
    '  }',
    '}',
    '',
...
export function serializeHTMLEventMap(components: ComponentMeta[]): string {
  const events = components
    ...
    .map(
      event => `    "${event.name}": CustomEvent<${event.typeInfo.resolved}>;`,
    )
    .join('\n');

  return ['  interface HTMLElementEventMap {', events, '  }'].join('\n');
}
...
export function serializeTypeImports(
  ...
  const typeImportsGroupedByFilePath = typeImports.reduce(
    (prevImports, typeImport) => ({
      ...prevImports,
      [typeImport.path]: [...(prevImports[typeImport.path] ?? []), typeImport],
    }),
  ...
  return Object.keys(typeImportsGroupedByFilePath)
    .map(filePath => {
      const importPath = isAbsolute(filePath)
        ? normalizePath(`./${relative(dirname(targetPath), filePath)}`).replace(
            '.ts',
            '',
          )
        : filePath;
  ...
  const membersText = members
    ...
    .reduce((serializedMembers, meta) => {
      if ((meta.documentation?.length ?? 0) > 0 || meta.docTags.length > 0) {
        ...
        serializedMembers.push(
          ...splitLineBreaks(meta.documentation ?? '').map(
            line => `     * ${line}`,
          ),
        );
        serializedMembers.push(
      ...
      const name =
        isJSX && isEvent ? `on${upperCaseFirstChar(meta.name)}` : meta.name;
      const optional = isJSX
        ? !(meta as PropMeta).required
        : (meta as PropMeta).optional;
      const aliasedType = resolveAliasForComponentMember(
        component,
        meta.typeInfo,
      );
      const type =
        isJSX && isEvent
          ? `(event: CustomEvent<${aliasedType}>) => void`
          : aliasedType;
