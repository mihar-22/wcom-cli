...
import { log, LogLevel, reportDiagnosticByLine } from '../../../utils/log';
...
export async function transpileModuleOnce(path: string): Promise<unknown> {
  const sourceText = (await readFile(path)).toString();
  const transpiledResult = transpileModule(sourceText, {
    compilerOptions: {
      ...defaultOptions,
      module: ModuleKind.CommonJS,
    },
  });
  return requireFromString(transpiledResult.outputText).default;
}
